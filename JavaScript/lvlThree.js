...
let shortPlatform = document.images.namedItem = '../gallery/shortPlatform.png'

let trap = document.images.namedItem = '../gallery/trap.png'
...
let star = document.images.namedItem = '../gallery/star.png'
...
class Player {
    ...
    jump() {
        if (this.jumpCount < this.maxJumps && player.position.y < 440) {
            this.velocity.y = -10; // Jump force
            this.jumpCount++;
            document.querySelector('.welcome').style.display = 'none'
        } else if (player.position.y > 435) {
...
let trapImage = createImage(trap)
...
let platforms = [
    ...
    new Platform({ x: platformImage.width * 2 + 100, y: 370, image: shortPlatformImage }), // short

    new Platform({ x: platformImage.width * 2 + 220, y: 315, image: trapImage }), // trap

    new Platform({ x: platformImage.width * 2 + 350, y: 270, image: shortPlatformImage }), // short
    ...
    new Platform({ x: platformImage.width * 5 + 370, y: 350, image: winingBoxImage }), // winBox
...
function animate() {
    ...
    if (player.position.x == platforms[3].x) {
        console.log('lost');
    }
